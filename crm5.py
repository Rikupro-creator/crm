...
class DatabaseManager:
    def __init__(self):
        ...
        self.db_dir = Path("client_databases")
        self.db_dir.mkdir(exist_ok=True)
        
        # Initialize the main authentication database
        self.auth_db_path = Path("auth.db")
        self.auth_db = sqlite3.connect(str(self.auth_db_path), check_same_thread=False)
        self.init_auth_db()

    def init_auth_db(self):
        """Initialize the authentication database"""
        c = self.auth_db.cursor()
    ...
    def create_client_database(self, company_name):
        """Create a new database for a client"""
        # Create a unique database name based on company name and timestamp
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        db_name = f"client_{hashlib.md5((company_name + timestamp).encode()).hexdigest()[:10]}.db"
        db_path = self.db_dir / db_name
        
        # Initialize the client's database
        conn = sqlite3.connect(str(db_path))
        c = conn.cursor()
        
        # Initialize all tables
        self._init_client_tables(c)
    ...
    def _init_client_tables(self, cursor):
        """Initialize all tables for a new client database"""
        # Core tables
        tables = {
            'customers': '''
...
def get_client_db():
    """Get database connection for the current client"""
    if hasattr(st.session_state, 'client_db'):
        db_path = Path("client_databases") / st.session_state.client_db
        if db_path.exists():
...
def login():
    ...
    if not st.session_state.logged_in:
        ...
        with login_tab:
            ...
            if st.button("Login"):
                c = st.session_state.db_manager.auth_db.cursor()
                c.execute("SELECT password, db_name, company_name FROM users WHERE username=?", (username,))
                result = c.fetchone()
                
                if result and result[0] == hash_password(password):
                    st.session_state.logged_in = True
                    st.session_state.username = username
                    st.session_state.client_db = result[1]
        ...
        with register_tab:
            ...
            if st.button("Register"):
                if not all([new_username, new_password, company_name]):
                    st.error("All fields are required")
                    return False
                
                try:
                    # Create client's database
                    db_name = st.session_state.db_manager.create_client_database(company_name)
                    
                    # Register user
                    c = st.session_state.db_manager.auth_db.cursor()
                    c.execute("""INSERT INTO users (username, password, company_name, db_name) 
                                VALUES (?, ?, ?, ?)""",
                             (new_username, hash_password(new_password), company_name, db_name))
                    st.session_state.db_manager.auth_db.commit()
...
def init_client_tables():
    """Initialize or update tables for a client's database"""
    conn = get_client_db()
    if conn:
        c = conn.cursor()
...
def show_dashboard():
    ...
    # [Rest of the dashboard code with updated database connections...]
...
def team_collaboration():
    ...
    with st.form("internal_message"):
        receiver = st.selectbox("To", pd.read_sql_query("SELECT username FROM users", get_client_db())['username'])
...
def main():
    ...
    # Initialize client's database tables
    init_client_tables()

    st.title(f"CRM System - {st.session_state.username}")
    ...
    # [Rest of the main function remains the same, but using get_client_db()]
